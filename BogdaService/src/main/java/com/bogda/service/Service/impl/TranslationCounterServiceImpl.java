...
public class TranslationCounterServiceImpl extends ServiceImpl<TranslationCounterMapper, TranslationCounterDO> implements ITranslationCounterService {
    ...
    public Boolean updateCharsByShopName(String shopName, String accessToken, String gid, Integer chars) {
        ...
        // 根据gid，判断是否符合添加额度的条件
        TraceReporterHolder.report("TranslationCounterServiceImpl.updateCharsByShopName", "updateCharsByShopName 用户： " + shopName + " gid: " + gid + " chars: " + chars + " accessToken: " + accessToken);

        // 根据传来的gid获取， 判断调用那个方法，查询相关订阅信息
        String query;
        if (gid.contains("AppPurchaseOneTime")) {
            TraceReporterHolder.report("TranslationCounterServiceImpl.updateCharsByShopName", "一次性购买 用户： " + shopName);
            query = ShopifyRequestUtils.getSingleQuery(gid);
        } else {
            TraceReporterHolder.report("TranslationCounterServiceImpl.updateCharsByShopName", "计划购买 用户： " + shopName);
        ...
        TraceReporterHolder.report("TranslationCounterServiceImpl.updateCharsByShopName", "addCharsByShopNameAfterSubscribe " + shopName + " 用户 订阅信息 ：" + shopifyByQuery + " query: " + query);


        // 判断和解析相关数据
        JSONObject queryValid = isQueryValid(shopifyByQuery);
        if (queryValid == null) {
            TraceReporterHolder.report("TranslationCounterServiceImpl.updateCharsByShopName", "updateCharsByShopName " + shopName + " 用户  errors queryValid : " + queryValid);
            return false;
        }
    ...
    public Boolean deleteTrialCounter(String shopName) {
        ...
        if (translationCounterDO != null && translationCounterDO.getOpenAiChars() == 1) {
            TraceReporterHolder.report("TranslationCounterServiceImpl.deleteTrialCounter", "deleteTrialCounter " + shopName + " 用户 删除试用额度 " + translationCounterDO.getGoogleChars());
