...
public class TranslationCounterService {
    ...
    public Boolean updateOnceCharsByShopName(String shopName, String accessToken, String gid, Integer chars) {
        ...
        //根据gid，判断是否符合添加额度的条件
        TraceReporterHolder.report("TranslationCounterService.updateOnceCharsByShopName", "updateCharsByShopName 用户： " + shopName + " gid: " + gid + " chars: " + chars + " accessToken: " + accessToken);
    ...
    public Boolean addCharsByShopNameAfterSubscribe(String shopName, TranslationCharsVO translationCharsVO) {
        ...
        TraceReporterHolder.report("TranslationCounterService.addCharsByShopNameAfterSubscribe", "addCharsByShopNameAfterSubscribe " + shopName + " 用户 订阅信息 ：" + shopifyByQuery);
        //判断和解析相关数据
        ...
        TraceReporterHolder.report("TranslationCounterService.addCharsByShopNameAfterSubscribe", "addCharsByShopNameAfterSubscribe " + shopName + " 用户 免费试用天数 ：" + trialDays + " name: " + name + " status: " + status + " 用户 订阅计划表 ：" + charsOrdersDO);
        ...
        if (name.equals(charsOrdersDO.getName()) && status.equals(charsOrdersDO.getStatus()) && trialDays > 0) {
            TraceReporterHolder.report("TranslationCounterService.addCharsByShopNameAfterSubscribe", "addCharsByShopNameAfterSubscribe " + shopName + " 用户 第一次免费试用 ：" + translationCharsVO.getSubGid());
        ...
        //添加额度
        TraceReporterHolder.report("TranslationCounterService.addCharsByShopNameAfterSubscribe", "addCharsByShopNameAfterSubscribe " + shopName + " 用户 计划名 ：" + charsOrdersDO.getName() + " name: " + name + " status: " + status);
