...
public class AliyunSlsIntegration {
    ...
    public boolean writeLogs(String topic, String source, Map<String, String> logs) {
        if (client == null) {
            System.out.println("FatalException AliyunSlsIntegration 客户端未初始化");
            return false;
        }
        
        if (project == null || logstore == null) {
            System.out.println("FatalException AliyunSlsIntegration PROJECT或LOGSTORE未配置");
            return false;
        }
        ...
        try {
            ...
            System.out.println("AliyunSlsIntegration 写入日志成功，topic: " + topic);
            return true;
        } catch (LogException e) {
            System.out.println("FatalException AliyunSlsIntegration 写入日志失败: " + e.getMessage());
            return false;
        } catch (Exception e) {
            System.out.println("FatalException AliyunSlsIntegration 写入日志异常: " + e.getMessage());
            return false;
        }
    }
    ...
    public List<Map<String, String>> readLogs(int from, int to, String query) {
        ...
        try {
            GetLogsRequest request = new GetLogsRequest(project, logstore, from, to, "", query, 100, 0, false);
            ...
            if (response != null && response.GetCount() > 0) {
                for (int i = 0; i < response.GetCount(); i++) {
                    ...
                    List<LogContent> contents = response.GetLogs().get(i).mLogItem.GetLogContents();
            ...
            System.out.println("AliyunSlsIntegration 读取日志成功，数量: " + result.size());
            return result;
        } catch (LogException e) {
            System.out.println("FatalException AliyunSlsIntegration 读取日志失败: " + e.getMessage());
            return new ArrayList<>();
        } catch (Exception e) {
            System.out.println("FatalException AliyunSlsIntegration 读取日志异常: " + e.getMessage());
            return new ArrayList<>();
        }
    }

    public List<Map<String, String>> readAggLogs(int from, int to, String query) {
