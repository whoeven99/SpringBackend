...
public interface GlossaryMapper extends BaseMapper<GlossaryDO> {
    ...
    @Select("select id,shop_name, source_text, target_text, range_code, case_sensitive, status from glossary where shop_name = #{shopName}")
    ...
    @Select("select id,shop_name, source_text, target_text, range_code, case_sensitive, status from glossary where shop_name = #{shopName} and source_text = #{sourceText}")
