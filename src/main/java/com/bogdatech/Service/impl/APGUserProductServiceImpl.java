...
public class APGUserProductServiceImpl extends ServiceImpl<APGUserProductMapper, APGUserProductDO> implements IAPGUserProductService {
    @Override
    public Boolean updateProductVersion(Long userId, String productId, String des, String productType) {
        ...
        if (apgUserProductDO == null) {
            ...
            newDO.setProductType(productType);
            return baseMapper.insert(newDO) > 0;
        }

        //查到了，更新这条数据
        return baseMapper.updateProductVersion(userId, productId, des, productType);
    }
}
