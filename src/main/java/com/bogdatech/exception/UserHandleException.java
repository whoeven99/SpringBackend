...
public class UserHandleException {
    ...
    public BaseResponse resolveException(Exception ex){
        ...
        if (ex instanceof ClientException){
            ...
            return new BaseResponse(clientException.getSuccess(),clientException.getMessage(),null);
        }

        //如果拦截的异常不是我们自定义的异常
        System.out.println("-----------异常错误信息---------");
        log.error("Exception failed by {}",ex);
        return new BaseResponse(SERVER_ERROR.getSuccess(),SERVER_ERROR.getErrorMessage(),null);
    }
    ...
    public BaseResponse handleMethodArgumentNotValidException  (MethodArgumentNotValidException ex){
        ...
        return new BaseResponse(SERVER_ERROR.getSuccess(),message,null);
    }
}
