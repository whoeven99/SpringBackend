...
//            appInsights.trackTrace("resp: " + resp.toString() + "resp_id: " + resp.getRequestId());
    ...
    public static String hunYuanPromptTranslate(String sourceText, String prompt, CharacterCountUtils countUtils, String model) {
        final int maxRetries = 3;
        final long baseDelayMillis = 1000; // 初始重试延迟为 1 秒
...
//            appInsights.trackTrace("resp: " + resp.toString() + "resp_id: " + resp.getRequestId());
    ...
    public static String hunYuanUserTranslate(String sourceText, CharacterCountUtils countUtils, String model) {
        ...
        messages[0].setContent(sourceText);
        req.setMessages(messages);
        req.setStream(false); // 非流式调用，设为 true 可启用流式返回
