...
public class TranslationParametersRedisService {
    ...
    public void translatedStatusAndSendEmail(String shopName, String target, String source) {
        ...
        if (writingData != null && writingData.get(WRITE_DONE) != null && writingData.get(WRITE_TOTAL) != null && writingData.get(WRITE_DONE) >= writingData.get(WRITE_TOTAL)) {
            ...
            InitialTranslateTasksDO initialTranslateTasksDO = initialTranslateTasksMapper.selectOne(new QueryWrapper<InitialTranslateTasksDO>().select("TOP 1 *").eq("status", 1).eq("deleted", false).eq("send_email", false).eq("shop_name", shopName).eq("target", target).eq("source", source));
            appInsights.trackTrace("translationDataToSave initialTranslateTasksDO : " + initialTranslateTasksDO);
