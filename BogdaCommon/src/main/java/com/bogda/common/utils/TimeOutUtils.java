    ...
    public static <T> T callWithTimeoutAndRetry(Supplier<T> task,
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            try {
                ...
                future.cancel(true);
                lastException = e;
                CaseSensitiveUtils.appInsights.trackTrace("FatalException task 调用超时（" + timeout + " " + unit + "），正在重试... [第" + attempt + "次]");
                CaseSensitiveUtils.appInsights.trackException(e);
            } catch (Exception e) {
                future.cancel(true);
                lastException = e;
                CaseSensitiveUtils.appInsights.trackTrace("FatalException 调用异常: " + e.getMessage() + "，正在重试... [第" + attempt + "次]");
                CaseSensitiveUtils.appInsights.trackException(e);
            } finally {
