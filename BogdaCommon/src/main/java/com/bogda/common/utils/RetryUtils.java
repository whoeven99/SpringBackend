        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt < maxRetries) {
                    CaseSensitiveUtils.appInsights.trackTrace("retryWithParam 第 " + attempt + " 次失败，等待 " + delay + " 毫秒后重试...");
                    Thread.sleep(delay);
                    delay = Math.min(delay * 2, maxDelayMillis);
                }
            } catch (Exception e) {
                CaseSensitiveUtils.appInsights.trackException(e);
                CaseSensitiveUtils.appInsights.trackTrace("retryWithParam 执行出错（第 " + attempt + " 次）：" + e.getMessage());
