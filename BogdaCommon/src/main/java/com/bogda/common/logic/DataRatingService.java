...
public class DataRatingService {
    ...
    public Map<String, Integer> getTranslationStatus(String shopName, String source) {
        ...
        String shopifyByQuery = shopifyService.getShopifyData(shopName, usersDO.getAccessToken(), TranslateConstants.API_VERSION_LAST, ShopifyRequestBody.getShopLanguageQuery());
        ...
        for (int i = 0; i < shopLocalesArr.size(); i++) {
            ...
            if (Boolean.FALSE.equals(published) && statusMap.get(ApiCodeUtils.getLanguageName(locale)) >= 1) {
                //翻译后的语言没有发布
        ...
        for (TranslatesDO translatesDO : list
            ...
            if (statusMap.containsKey(ApiCodeUtils.getLanguageName(translatesDO.getTarget()))) {
                statusMap.put(ApiCodeUtils.getLanguageName(translatesDO.getTarget()), translatesDO.getStatus());
            }
        }

        // 5, 将source对应的语言删除
        statusMap.remove(ApiCodeUtils.getLanguageName(source));
        return statusMap;
    }
