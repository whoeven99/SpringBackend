...
import com.bogda.common.integration.RedisIntegration;
...
public class RedisDataReportService {
    ...
    public void saveUserDataReport(String shopName, UserDataReportVO userDataReportVO) {
        ...
        String dataReportKey = RedisKeyUtils.DATA_REPORT_KEY_TEMPLATE.replace("{shopName}", shopName)
                .replace("{language}", userDataReportVO.getStoreLanguage()[0])
                .replace("{yyyyMMdd}", format);

        //对传入的client_id做去重，然后再加一
        String clientIdSetKey = RedisKeyUtils.CLIENT_ID_SET.replace("{shopName}", shopName)
                .replace("{language}", userDataReportVO.getStoreLanguage()[0])
                .replace("{yyyyMMdd}", format)
                .replace("{eventName}", userDataReportVO.getEventName());
        String setKeys = RedisKeyUtils.DATA_REPORT_KEY_TEMPLATE_KEYS.replace("{shopName}", shopName);
        ...
        if (flag) {
            redisIntegration.expire(clientIdSetKey, RedisKeyUtils.DAY_1);
            Long count = redisIntegration.incrementHash(dataReportKey, userDataReportVO.getEventName(), 1L);
            if (count != null && count == 1) {
                redisIntegration.expire(dataReportKey, RedisKeyUtils.DAY_15);
            }
        }
    }
    ...
    public String getUserDataReport(String shopName, Timestamp timestamp, int dayData) {
        ...
        if (redisIntegrationSet == null || redisIntegrationSet.isEmpty()) {
            CaseSensitiveUtils.appInsights.trackTrace("getUserDataReport 没有找到 key, shopName=" + shopName);
            return null;
        }
        ...
        for (String languageCode : redisIntegrationSet) {
            ...
            for (int i = 0; i < dayData; i++) {
                ...
                String key = RedisKeyUtils.DATA_REPORT_KEY_TEMPLATE.replace("{shopName}", shopName)
        ...
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("getUserDataReport JSON序列化失败, shopName=" + shopName);
            return null;
        }
    }
}
