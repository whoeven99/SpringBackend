package com.bogda.common.logic.translate;
...
import com.bogda.common.integration.model.ShopifyGraphResponse;
import com.bogda.common.logic.GlossaryService;
...
import com.bogda.common.logic.translate.stragety.ITranslateStrategyService;
import com.bogda.common.logic.translate.stragety.TranslateStrategyFactory;
...
import com.bogda.common.model.controller.response.TypeSplitResponse;
...
import com.bogda.common.repository.repo.TranslateTaskV2Repo;
...
import static com.bogda.common.logic.TaskService.AUTO_TRANSLATE_MAP;
...
public class TranslateV2Service {
    ...
    public Map<String, Object> testTranslate(Map<String, Object> map) {
        ...
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("FatalException testTranslate JSON parsing failed: " + e.getMessage());
        ...
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("FatalException in testTranslate: " + e.getMessage());
        }
    ...
    public BaseResponse<Object> createInitialTask(ClickTranslateRequest request) {
        String shopName = request.getShopName();
        CaseSensitiveUtils.appInsights.trackTrace("createInitialTask : " + " shopName : " + shopName + request);
        ...
        List<String> resourceTypeList = moduleList.stream()
                ...
                .flatMap(module -> {
                    ...
                    if (list == null) {
                        CaseSensitiveUtils.appInsights.trackTrace("FatalException createInitialTask Warning: Unknown module: " + module);
                        return Stream.empty();
                    }
                    return list.stream();
                })
    ...
    public void isExistInDatabase(String shopName, String[] targets, String source, String accessToken) {
        ...
        if (!shopLocales.isArray()) {
            CaseSensitiveUtils.appInsights.trackTrace("syncShopifyAndDatabase: shopLocales is not an array.");
            return;
        }
            ...
            shopifyService.rotateAllShopifyGraph(shopName, module, userDO.getAccessToken(), 250, target, afterEndCursor,
                    (node -> {
                        if (node != null && !CollectionUtils.isEmpty(node.getTranslatableContent())) {
                            ...
                            CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 rotating Shopify: " + shopName + " module: " + module +
...
//                        List<TranslateTaskV2DO> existingTasks = translateTaskV2Repo.selectByResourceId(node.getResourceId());
                            ...
                            node.getTranslatableContent().forEach(translatableContent -> {
                                if (needTranslate(translatableContent, node.getTranslations(), module, initialTaskV2DO.isCover(), initialTaskV2DO.isHandle())) {
                                    ...
                                    try {
                                        ...
                                        CaseSensitiveUtils.appInsights.trackException(e);
                                        CaseSensitiveUtils.appInsights.trackTrace("FatalException initialToTranslateTask insert error " + e.getMessage());
                                    }
                                }
                            });
                        }
            ...
            CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 rotate Shopify done: " + shopName + " module: " + module);
        }

        // 更新数据库并记录初始化时间
        CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 initialToTranslateTask done: " + shopName);
    ...
    private boolean isSingleHtml(ShopifyGraphResponse.TranslatableResources.Node.TranslatableContent translatableContent,
        ...
        if ("body".equals(translatableContent.getKey()) || "body_html".equals(translatableContent.getKey())
            ...
            return JsoupUtils.isHtml(translatableContent.getValue());
        }
        return false;
    }
    ...
    public void translateEachTask(InitialTaskV2DO initialTaskV2DO) {
        ...
        while (randomDo != null) {
            CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 translating shop: " + shopName + " randomDo: " + randomDo.getId());
        ...
        CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 translating done: " + shopName);
    ...
    public void saveToShopify(InitialTaskV2DO initialTaskV2DO) {
        ...
        while (randomDo != null) {
            CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 saving shopify shop: " + shopName + " randomDo: " + randomDo.getId());
            ...
            String strResponse = shopifyHttpIntegration.sendShopifyPost(
                    shopName, token, TranslateConstants.API_VERSION_LAST, ShopifyRequestUtils.registerTransactionQuery(), node);
            JSONObject jsonObject = JSONObject.parseObject(strResponse);
            if (jsonObject != null && jsonObject.getJSONObject("data") != null) {
                CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 saving success: " + shopName +
            ...
            CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 saving SHOPIFY: " + shopName + " size: " + taskList.size());
        }
    ...
    public boolean autoTranslateV2(String shopName, String source, String target) {
        ...
        if (usersDO == null) {
            CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 已卸载 用户: " + shopName);
            return false;
        }

        if (usersDO.getUninstallTime() != null) {
            if (usersDO.getLoginTime() == null) {
                CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 卸载了未登陆 用户: " + shopName);
                return false;
            } else if (usersDO.getUninstallTime().after(usersDO.getLoginTime())) {
                CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 卸载了时间在登陆时间后 用户: " + shopName);
                return false;
            }
        }
        ...
        // 判断注册时间
        if (usersDO.getLoginTime() == null) {
            CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 用户未登录 或 登录时间为空: " + shopName);
            return false;
        }
        ...
        CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 loginHour: " + loginHour + " currentHour: " + currentHour + " shop: " + shopName);

        // 只有当登录小时与当前小时一致时才继续执行，其他情况按分片逻辑跳过
        if (loginHour != currentHour) {
            CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 非当前小时，不执行自动翻译 shop: " + shopName);
            return false;
        }

        Integer maxToken = userTokenService.getMaxToken(shopName);
        Integer usedToken = userTokenService.getUsedToken(shopName);
        CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 maxToken: " + maxToken + " usedToken: " + usedToken + " shop: " + shopName);
        // 如果字符超限，则直接返回字符超限
        if (usedToken >= maxToken) {
            CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 字符超限 用户: " + shopName);
            return false;
        }

        // 判断这条语言是否在用户本地存在
        String shopifyByQuery = shopifyService.getShopifyData(shopName, usersDO.getAccessToken(),
                TranslateConstants.API_VERSION_LAST, ShopifyRequestBody.getShopLanguageQuery());
        CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 获取用户本地语言数据: " + shopName + " 数据为： " + shopifyByQuery);
        if (shopifyByQuery == null) {
            CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 FatalException 获取用户本地语言数据失败 用户: " + shopName + " ");
            return false;
        }
        ...
        if (!shopifyByQuery.contains(userCode)) {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 用户本地语言数据不存在 用户: " + shopName + " target: " + target);
            return false;
        }

        CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 任务准备创建 " + shopName + " target: " + target);
        createAutoTask(shopName, source, target);
        CaseSensitiveUtils.appInsights.trackTrace("autoTranslateV2 任务创建成功 " + shopName + " target: " + target);
        return true;
    }

    public void cleanTask(InitialTaskV2DO initialTaskV2DO) {
        CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 cleanTask start clean task: " + initialTaskV2DO.getId());
        while (true) {
            int deleted = translateTaskV2Repo.deleteByInitialTaskId(initialTaskV2DO.getId());
            CaseSensitiveUtils.appInsights.trackTrace("TranslateTaskV2 cleanTask delete: " + deleted);
            if (deleted <= 0) {
                break;
            }
        }
    ...
    private static TypeSplitResponse splitByType(String targetType, List<TranslateResourceDTO> resourceList) {
        ...
        if (targetType == null) {
            ...
            Set<String> emailResources = new HashSet<>(TranslateResourceDTO.EMAIL_MAP.values());
        ...
        if (index == -1) {
            CaseSensitiveUtils.appInsights.trackTrace("errors 错误：未找到 type 为 '" + targetType + "' 的资源");
            after = resourceList;
            for (TranslateResourceDTO resource : after) {
                afterSet.add(TranslateResourceDTO.EMAIL_MAP.get(resource.getResourceType()));
            }
        ...
        if (!before.isEmpty()) {
            for (TranslateResourceDTO resource : before) {
                if (TranslateResourceDTO.EMAIL_MAP.containsKey(resource.getResourceType())) {
                    beforeSet.add(TranslateResourceDTO.EMAIL_MAP.get(resource.getResourceType()));
                }
            }
        }
    ...
    private boolean needTranslate(ShopifyGraphResponse.TranslatableResources.Node.TranslatableContent translatableContent,
        ...
        if (TranslateConstants.METAFIELD.equals(module)) {
            ...
            if (JudgeTranslateUtils.SUSPICIOUS_PATTERN.matcher(value).matches() || JudgeTranslateUtils.SUSPICIOUS2_PATTERN.matcher(value).matches()) {
                return false;
            }
