...
public class PCUsersService {
    ...
    public BaseResponse<Object> addPurchasePoints(String shopName, Integer chars, String orderId) {
        ...
        if (!"null".equals(redisOrderId)) {
            CaseSensitiveUtils.appInsights.trackTrace("PC addCharsByShopName 用户 " + shopName + " redisOrderId : " + redisOrderId);
            return new BaseResponse<>().CreateSuccessResponse(true);
        }

        boolean flag = pcUsersRepo.updatePurchasePointsByShopName(shopName, userByShopName.getAccessToken(), orderId, chars);
        if (flag) {
            CaseSensitiveUtils.appInsights.trackTrace("PC addPurchasePoints 添加额度成功 " + shopName + " chars: " + chars);
            return new BaseResponse<>().CreateSuccessResponse(true);
        }
    ...
    public BaseResponse<Object> uninstall(String shopName) {
        ...
        if (!isDeduct) {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("PC uninstall 用户 " + shopName + " 扣除额度 ：" + deduct);
        }
    ...
    public BaseResponse<Object> addCharsByShopNameAfterSubscribe(String shopName, TranslationCharsVO translationCharsVO) {
        ...
        if (!"null".equals(redisOrderId)) {
            CaseSensitiveUtils.appInsights.trackTrace("PC addCharsByShopName 用户 " + shopName + " redisOrderId: " + redisOrderId  + " orderId : " + translationCharsVO.getSubGid());
            return new BaseResponse<>().CreateErrorResponse(false);
        }
        ...
        CaseSensitiveUtils.appInsights.trackTrace("PC addCharsByShopNameAfterSubscribe " + shopName + " 用户 订阅计划表 ：" + pcOrdersDO.toString());
        ...
        CaseSensitiveUtils.appInsights.trackTrace("PC addCharsByShopNameAfterSubscribe " + shopName + " 用户 免费试用天数 ：" + trialDays + " name: " + name + " status: " + status);
        ...
        if (name.equals(pcOrdersDO.getName()) && status.equals(pcOrdersDO.getStatus()) && trialDays > 0) {
            CaseSensitiveUtils.appInsights.trackTrace("PC addCharsByShopNameAfterSubscribe " + shopName + " 用户 第一次免费试用 ：" + translationCharsVO.getSubGid());
        ...
        // 添加额度
        CaseSensitiveUtils.appInsights.trackTrace("addCharsByShopNameAfterSubscribe " + shopName + " 用户 计划名 ：" + pcOrdersDO.getName() + " name: " + name + " status: " + status);
