...
public class APGUserGeneratedTaskService {
    ...
    public GenerateProgressBarVO getUserData(String shopName) {
        ...
        try {
            ...
            generateProgressBarVO = TypeConversionUtils.apgUserGeneratedTaskDOToGenerateProgressBarVO(taskDO, totalCount, unfinishedCount);
            ...
            CaseSensitiveUtils.appInsights.trackTrace(shopName + " 用户 " + userDO.getId() + " 的taskData有问题 errors ： " + e);
    ...
    public void batchGenerateDescription(APGUsersDO usersDO, String shopName, GenerateDescriptionsVO generateDescriptionsVO) {
        ...
        for (String productId : generateDescriptionsVO.getProductIds()) {
            GenerateDescriptionVO generateDescriptionVO = TypeConversionUtils.generateDescriptionsVOToGenerateDescriptionVO(productId, generateDescriptionsVO);
            try {
                ...
                CaseSensitiveUtils.appInsights.trackTrace(shopName + " 用户 批量翻译json化失败 errors 数据为 ： " + generateDescriptionVO + "  " + e);
//                appInsights.trackTrace(shopName + " 用户 批量翻译json化失败 errors 数据为 ： " + generateDescriptionVO + "  " + e);
        ...
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace(shopName + " 用户 批量翻译json化失败 errors 数据为 ： " + generateEmailVO + "  " + e);
            CaseSensitiveUtils.appInsights.trackException(e);
