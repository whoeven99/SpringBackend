...
import static com.bogda.common.logic.APGUserGeneratedTaskService.*;
...
public class GenerateDescriptionService {
    ...
    public String generateDescription(APGUsersDO usersDO, GenerateDescriptionVO generateDescriptionVO, CharacterCountUtils counter, Integer userMaxLimit, ProductDTO product) {
        ...
        //生成提示词
        String prompt = PlaceholderUtils.buildDescriptionPrompt(product.getProductTitle(), product.getProductType(), product.getProductDescription(), generateDescriptionVO.getSeoKeywords(), product.getImageUrl(), product.getImageAltText(), generateDescriptionVO.getTextTone(), templateById.getTemplateType(), generateDescriptionVO.getBrandTone(), templateById.getTemplateData(), generateDescriptionVO.getLanguage(), generateDescriptionVO.getContentType(), generateDescriptionVO.getBrandWord(), generateDescriptionVO.getBrandSlogan());
        CaseSensitiveUtils.appInsights.trackTrace(usersDO.getShopName() + " 用户 " + product.getId() + " 的提示词为 ： " + prompt);
        //调用大模型翻译
        //如果产品图片为空，换模型生成
        String des;
...
//        每次生成都要更新一下版本记录和生成数据
    ...
    public ProductDTO getProductsQueryByProductId(String productId, String shopName, String accessToken) {
        String productDataQuery = ShopifyRequestBody.getProductDataQuery(productId);
        ...
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("getProductsQueryByProductId errors : " + e);
    ...
    public TemplateDTO getTemplateById(Long templateId, Long userId, Boolean templateType) {
        ...
        if (templateType) {
            ...
            return TypeConversionUtils.userTemplateToTemplateDTO(one);
        }else {
            //获取官方模板
            ...
            return TypeConversionUtils.officialTemplateToTemplateDTO(one);
        }
    }

    public APGAnalyzeDataVO analyzeDescriptionData(String description, String generation, String seoKeywords) {
        //根据生成前数据和生成后数据分析
        double desInt = StringUtils.countWords(description);
        double proDesInt = StringUtils.countWords(generation);
        int seoKeywordsInt = StringUtils.countWords(seoKeywords);
