...
import com.bogda.common.repository.repo.PCSubscriptionQuotaRecordRepo;
...
public class PCSubscriptionQuotaRecordService {
    ...
    public BaseResponse<Object> addSubscriptionQuotaRecord(String shopName, PCSubscriptionQuotaRecordDO pcSubscriptionQuotaRecordDO) {
        ...
        CaseSensitiveUtils.appInsights.trackTrace("addSubscriptionQuotaRecord " + pcSubscriptionQuotaRecordDO.getSubscriptionId() + " 这条数据只是判断是否要往数据库存值 quotaRecordDO: " + quotaRecordDO);
        if (quotaRecordDO != null) {
            CaseSensitiveUtils.appInsights.trackTrace("addSubscriptionQuotaRecord " + pcSubscriptionQuotaRecordDO.getSubscriptionId() + " 数据库中含有这条数据，就不插入了");
