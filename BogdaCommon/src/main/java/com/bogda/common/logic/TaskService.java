...
import com.bogda.common.model.controller.request.UserPriceRequest;
...
import com.bogda.common.repository.repo.*;
...
public class TaskService {
    ...
    public void judgeAddChars() {
        ...
        for (UserPriceRequest userPriceRequest : usedList
            ...
            try {
                ...
                CaseSensitiveUtils.appInsights.trackTrace("judgeAddChars 用户： " + userPriceRequest.getShopName() + " 获取数据 errors : " + e);
            }
        }
    ...
    public void judgeSubscriptionStatus(List<UserPriceRequest> usedList) {
        ...
        for (UserPriceRequest order : latestOrderMap.values()) {
            try {
                ...
                if (node == null) {
                    CaseSensitiveUtils.appInsights.trackTrace("judgeAddChars " + order.getShopName() + " 获取不到计划的相关数据，获取为null");
                    continue;
                }
                ...
                if (!"ACTIVE".equals(status)) {
                    ...
                    CaseSensitiveUtils.appInsights.trackTrace("judgeAddChars " + order.getShopName() + " 计划过期，将用户计划改为2 " + " 修改状态: " + i);
                }
            } catch (Exception e) {
                CaseSensitiveUtils.appInsights.trackException(e);
                CaseSensitiveUtils.appInsights.trackTrace("judgeAddChars 用户： " + order.getShopName() + " 获取订阅计划数据 errors : " + e);
            }
        }
    }
    ...
    public JSONObject analyzeOrderData(String subscriptionId, String accessToken, String shopName) {
        String query = ShopifyRequestBody.getSubscriptionQuery(subscriptionId);
        ...
        if (root == null || root.isEmpty()) {
            CaseSensitiveUtils.appInsights.trackTrace(shopName + " 定时任务根据订单id: " + subscriptionId + "获取数据失败" + " token: " + accessToken);
            return null;
        }
    ...
    public void addCharsByUserData(UserPriceRequest userPriceRequest) {
        ...
        if (node == null) {
            CaseSensitiveUtils.appInsights.trackTrace("addCharsByUserData 用户： " + userPriceRequest.getShopName() + " 获取不到计划的相关数据，获取为null " + userPriceRequest);
            return;
        }
...
//            appInsights.trackTrace("满足条件，执行添加字符的逻辑");
            ...
            CaseSensitiveUtils.appInsights.trackTrace("addCharsByUserData 用户： " + userPriceRequest.getShopName() + " 添加字符额度： " + chars + " 是否成功： " + flag);
    ...
    public static final List<String> AUTO_TRANSLATE_MAP = new ArrayList<>(Arrays.asList(
            ...
            TranslateConstants.ONLINE_STORE_THEME_JSON_TEMPLATE, TranslateConstants.ONLINE_STORE_THEME_SECTION_GROUP,
            TranslateConstants.ONLINE_STORE_THEME_SETTINGS_CATEGORY, TranslateConstants.ONLINE_STORE_THEME_SETTINGS_DATA_SECTIONS,
            TranslateConstants.ONLINE_STORE_THEME_LOCALE_CONTENT, TranslateConstants.COLLECTION, TranslateConstants.PRODUCT,
            TranslateConstants.PRODUCT_OPTION, TranslateConstants.PRODUCT_OPTION_VALUE, TranslateConstants.BLOG,
            TranslateConstants.ARTICLE, TranslateConstants.PAGE, TranslateConstants.METAFIELD, TranslateConstants.SHOP_POLICY,
        ...
        for (UserTrialsDO userTrialsDO : notTrialExpired) {
            ...
            if (now.after(trialEnd)) {
                ...
                if (latestActiveSubscribeId == null) {
                    CaseSensitiveUtils.appInsights.trackTrace("freeTrialTask  latestActiveSubscribeId的数据为null，用户是：" + shopName);
                ...
                String subscriptionQuery = ShopifyRequestBody.getSubscriptionQuery(latestActiveSubscribeId);
                ...
                if (!"ACTIVE".equals(status)) {
                    try {
                        ...
                        CaseSensitiveUtils.appInsights.trackTrace(userTrialsDO.getShopName() + "用户  errors 修改用户计划失败: " + e.getMessage());
                    }
                    continue;
                }
                ...
                CaseSensitiveUtils.appInsights.trackTrace(shopName + " 用户 添加额度成功 ： " + charsByPlanName + " 计划为： " + name + " 是否成功： " + flag);
            }
        }
    }
    ...
    public void judgePCAppAddChars() {
        ...
        for (PCOrdersDO pcOrdersDO : orderList) {
            ...
            try {
                ...
                CaseSensitiveUtils.appInsights.trackException(e);
                CaseSensitiveUtils.appInsights.trackTrace("judgeAddChars 用户： " + pcOrdersDO.getShopName() + " 获取数据 errors : " + e);
            }
        }
    ...
    private void addPCCharsByUserData(String shopName, String accessToken, String subscriptionId, Timestamp userCreatedAt) {
        ...
        if (node == null) {
            CaseSensitiveUtils.appInsights.trackTrace("addCharsByUserData 用户： " + shopName + " 获取不到计划的相关数据，获取为null " + accessToken + " " + subscriptionId);
            return;
        }
        String name = node.getString("name");
        String status = node.getString("status");
...
//            appInsights.trackTrace("不满足条件");
        ...
        // 计算当前是第几个月
        int billingCycle = (int) ChronoUnit.DAYS.between(buyCreateInstant, now) / 30 + 1;
        CaseSensitiveUtils.appInsights.trackTrace("PC billingCycle = " + billingCycle);
        ...
        if (quotaRecordDO == null) {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("PC addCharsByUserData 用户： " + shopName + " 添加字符额度： " + chars + " 是否成功： " + flag);
    ...
    public void freeTrialTaskForImage() {
        ...
        for (PCUserTrialsDO pcUserTrialsDO : notTrialExpired) {
            ...
            if (now.after(trialEnd)) {
                ...
                if (latestActiveSubscribeId == null) {
                    CaseSensitiveUtils.appInsights.trackTrace("PC freeTrialTask latestActiveSubscribeId的数据为null，用户是：" + shopName);
                ...
                if (!"ACTIVE".equals(status)) {
                    try {
                        ...
                        CaseSensitiveUtils.appInsights.trackTrace(shopName + " 用户  errors PC 修改用户计划失败: " + e.getMessage());
                    }
                    continue;
                }
                ...
                CaseSensitiveUtils.appInsights.trackTrace(shopName + " 用户 PC 添加额度成功 ： " + charsByPlanName + " 计划为： " + name + " 是否成功： " + flag);
            }
        }
    }
}
