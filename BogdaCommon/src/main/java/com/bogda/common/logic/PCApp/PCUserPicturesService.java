...
public class PCUserPicturesService {
    ...
    public BaseResponse<Object> translatePic(String shopName, ImageTranslateVO imageTranslateVO) {
        CaseSensitiveUtils.appInsights.trackTrace("imageTranslate 用户 " + shopName + " sourceCode " + imageTranslateVO.getSourceCode() + " targetCode " + imageTranslateVO.getTargetCode() + " imageUrl " + imageTranslateVO.getImageUrl() + " accessToken " + imageTranslateVO.getAccessToken());
    ...
    public BaseResponse<Object> altTranslate(String shopName, AltTranslateVO altTranslateVO) {
        CaseSensitiveUtils.appInsights.trackTrace("altTranslate 用户 " + shopName + " sourceCode " + altTranslateVO.getTargetCode() + " targetCode " + altTranslateVO.getTargetCode() + " alt " + altTranslateVO.getAlt() + " accessToken " + altTranslateVO.getAccessToken());
        // 获取用户token，判断是否和数据库中一致再选择是否调用
        PCUsersDO pcUsersDO = pcUsersRepo.getUserByShopName(shopName);
        ...
        // 生成提示词
        String prompt = "请将以下文本翻译为如下语言：" + ApiCodeUtils.getLanguageName(altTranslateVO.getTargetCode());
