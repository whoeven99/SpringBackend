...
public class TranslationCounterService {
    ...
    public Boolean updateOnceCharsByShopName(String shopName, String accessToken, String gid, Integer chars) {
        ...
        //根据gid，判断是否符合添加额度的条件
        CaseSensitiveUtils.appInsights.trackTrace("updateCharsByShopName 用户： " + shopName + " gid: " + gid + " chars: " + chars + " accessToken: " + accessToken);
    ...
    public Boolean addCharsByShopNameAfterSubscribe(String shopName, TranslationCharsVO translationCharsVO) {
        ...
        CaseSensitiveUtils.appInsights.trackTrace("addCharsByShopNameAfterSubscribe " + shopName + " 用户 订阅信息 ：" + shopifyByQuery);
        //判断和解析相关数据
        ...
        CaseSensitiveUtils.appInsights.trackTrace("addCharsByShopNameAfterSubscribe " + shopName + " 用户 订阅计划表 ：" + charsOrdersDO.toString());
        ...
        CaseSensitiveUtils.appInsights.trackTrace("addCharsByShopNameAfterSubscribe " + shopName + " 用户 免费试用天数 ：" + trialDays + " name: " + name + " status: " + status);
        ...
        if (name.equals(charsOrdersDO.getName()) && status.equals(charsOrdersDO.getStatus()) && trialDays > 0) {
            CaseSensitiveUtils.appInsights.trackTrace("addCharsByShopNameAfterSubscribe " + shopName + " 用户 第一次免费试用 ：" + translationCharsVO.getSubGid());
            ...
            if (userTrialsDO == null) {
                ...
                CaseSensitiveUtils.appInsights.trackTrace("addCharsByShopNameAfterSubscribe " + shopName + " 用户 免费试用额度添加 ：" + charsByPlan + " 是否成功： " + update);
                return update;
            }
        } else {
        ...
        //添加额度
        CaseSensitiveUtils.appInsights.trackTrace("addCharsByShopNameAfterSubscribe " + shopName + " 用户 计划名 ：" + charsOrdersDO.getName() + " name: " + name + " status: " + status);
