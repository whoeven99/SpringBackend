...
import com.bogda.common.logic.redis.UserInitialRedisService;
...
public class UserService {
    ...
    public BaseResponse<Object> addUser(UsersDO usersDO) {
        ...
        if (i > 0) {
            ...
            Boolean flag1 = tencentEmailService.sendInitialUserEmail(137916L, templateData, MailChimpConstants.FIRST_INSTALL_SUBJECT, MailChimpConstants.TENCENT_FROM_EMAIL, usersDO.getEmail());

            //存数据库中
            Integer flag2 = emailService.saveEmail(new EmailDO(0, usersDO.getShopName(), MailChimpConstants.TENCENT_FROM_EMAIL, usersDO.getEmail(), MailChimpConstants.FIRST_INSTALL_SUBJECT, flag1 ? 1 : 0));
...
//            }
            CaseSensitiveUtils.appInsights.trackTrace("webhookDefaultTheme 用户主题改变 ： " + shopName + " name : " + themeName + " id : " + themeId );
        }
    ...
    public BaseResponse<Object> webhookDefaultLanguage(String shopName, ThemeAndLanguageVO data) {
        ...
        if (!defaultLanguageData.equals(userDefaultLanguage)) {
            ...
            userInitialRedisService.setUserDefaultLanguage(shopName, defaultLanguageData);
            CaseSensitiveUtils.appInsights.trackTrace("webhookDefaultLanguage 用户默认语言改变 ： " + shopName + " source : " + userDefaultLanguage + " language : " + defaultLanguageData );
        }
