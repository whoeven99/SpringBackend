...
import com.bogda.common.model.controller.request.UserPrivateRequest;
...
public class UserPrivateService {
    ...
    public BaseResponse<Object> saveOrUpdateUserData(UserPrivateRequest userPrivateRequest) {
        ...
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("saveOrUpdateUserData " + userPrivateRequest.getShopName() + " 保存用户数据失败：" + e.getMessage());
    ...
    public BaseResponse<Object> getUserData(UserPrivateRequest userPrivateRequest) {
        ...
        while (retries > 0) {
            try {
                ...
                retries--;
                if (retries == 0) {
                    CaseSensitiveUtils.appInsights.trackTrace("getUserData " + userPrivateRequest.getShopName() + " failed: " + e.getMessage());
                } else {
                    try {
    ...
    public Boolean deleteUserData(String shopName) {
        ...
        if (user == null) {
            CaseSensitiveUtils.appInsights.trackTrace("deleteUserData " + shopName + " 用户不存在");
            return false;
        }
        //删除用户在keyVault里面的数据
        try {
            shopName = StringUtils.replaceDot(shopName);
            secretClient.getDeletedSecret(shopName + "-" + "google");
        } catch (Exception e) {
            CaseSensitiveUtils.appInsights.trackException(e);
            CaseSensitiveUtils.appInsights.trackTrace("deleteUserData " + shopName + " 删除用户在keyVault里面的数据失败：" + e.getMessage());
        }
