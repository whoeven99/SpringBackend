...
//            AppInsightsUtils.printTranslateCost(totalToken, totalToken, totalToken);
            ...
            CaseSensitiveUtils.appInsights.trackTrace("信息：" + e.getMessage());
        }
        return null;
    }
    ...
    public static String getGoogleTranslationWithRetry(TranslateRequest request) {
        ...
        do {
            try {
                ...
                CaseSensitiveUtils.appInsights.trackTrace("翻译 API 调用失败，重试次数：" + retryCount + "，错误信息：" + e.getMessage() + " sourceText: " + request.getContent() + " target: " + request.getTarget());
            }
