...
//            appInsights.trackTrace("translateText: " + jsonString);
            ...
            if (translationList != null && !translationList.isEmpty()) {
                ...
                CaseSensitiveUtils.appInsights.trackTrace("Translation list is empty or not present.");
            }
        } catch (Exception e) {
            CaseSensitiveUtils.appInsights.trackTrace("huoShanTranslate " + e.getMessage());
        }
        return translation;
    }
    ...
    public byte[] huoShanImageTranslate(String imageUrl, String targetLanguage) {
        ...
        CaseSensitiveUtils.appInsights.trackTrace("huoShanImageTranslate imageUrl : " + imageUrl + " targetLanguage: " + targetLanguage);
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("huoShanImageTranslate 返回的数据 ： " + JSON.toJSONString(translateImageResponse.getResponseMetadata()));
            return Base64.getDecoder().decode(translateImageResponse.getImage());

        } catch (Exception e) {
            CaseSensitiveUtils.appInsights.trackTrace("huoShanImageTranslate " + e.getMessage());
        }
        return null;
    }
}
