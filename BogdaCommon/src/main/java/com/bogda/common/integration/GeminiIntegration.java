...
public class GeminiIntegration {
    ...
    public Pair<String, Integer> generateText(String model, String prompt) {
        // 发送对话请求
        try {
            CaseSensitiveUtils.appInsights.trackTrace("model : " + model + " translateText : " + prompt);
            ...
            CaseSensitiveUtils.appInsights.trackTrace("Gemini 提示词： " + prompt + " 生成文本： " + text + " 请求token: " + inputToken + " 生成token: " + outputToken + " 总token: " + allToken);
    ...
    public Pair<String, Integer> generateImage(String model, String prompt, byte[] picBytes, String mimeType) {
        try {
            CaseSensitiveUtils.appInsights.trackTrace("model : " + model + " translateText : " + prompt + " picBytes : " + picBytes + " mimeType : " + mimeType);
            ...
            CaseSensitiveUtils.appInsights.trackTrace("generateImage 模型说明: " + response.toString());
            byte[] translatedBytes = new byte[0];

            for (Part part : response.parts()) {
                if (part.text().isPresent()) {
                    CaseSensitiveUtils.appInsights.trackTrace("模型说明: " + part.text().get());
                }
                if (part.inlineData().isPresent()) {
                    var blob = part.inlineData().get();
                    if (blob.data().isPresent()) {
                        translatedBytes = blob.data().get();
                        CaseSensitiveUtils.appInsights.trackTrace("翻译后的图片已保存为: " + Base64.getEncoder().encodeToString(blob.data().get()));
                    }
                }
            }
            ...
            CaseSensitiveUtils.appInsights.trackTrace("Gemini 提示词： " + prompt + " 生成文本： " + Base64.getEncoder().encodeToString(translatedBytes) + " 请求token: " + inputToken + " 生成token: " + outputToken + " 总token: " + allToken);
