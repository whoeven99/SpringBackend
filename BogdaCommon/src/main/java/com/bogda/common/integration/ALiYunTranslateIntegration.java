...
public class ALiYunTranslateIntegration {
    ...
    public static Integer calculateBaiLianToken(String text) {
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("calculateBaiLianToken " + e.getMessage() + " 计数失败 text : " + text);
            return text.length();
        }
    }
...
//        return "qwen-max";
        ...
        try {
            GenerationResult call = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                        try {
                            ...
                            CaseSensitiveUtils.appInsights.trackTrace("FatalException userTranslate call errors ： " + e.getMessage() +
            ...
            CaseSensitiveUtils.appInsights.trackTrace("userTranslate 原文本：" + prompt + " 翻译成： " + content +
                    " token ali  all: " + totalToken + " input: " + inputTokens + " output: " + outputTokens);
            return new Pair<>(content, totalToken);
        } catch (Exception e) {
            CaseSensitiveUtils.appInsights.trackTrace("FatalException userTranslate errors ： " + e.getMessage() + " translateText : " + prompt);
            CaseSensitiveUtils.appInsights.trackException(e);
            return new Pair<>(null, 0);
        }
    }
    ...
    public String callWithPicMess(String prompt, Long userId, CharacterCountUtils counter, String picUrl, Integer userMaxLimit) {
        ...
        try {
            result = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                        try {
                            return conv.call(param);
                        } catch (Exception e) {
                            CaseSensitiveUtils.appInsights.trackTrace("每日须看 callWithPicMess 百炼翻译报错信息 errors ： " + e.getMessage() + " picUrl : " + picUrl + " 用户：" + userId);
                            CaseSensitiveUtils.appInsights.trackException(e);
                            return null;
                        }
                    },
            ...
            CaseSensitiveUtils.appInsights.trackTrace("callWithPicMess 用户 " + userId + " token ali-vl : " + content + " all: " + totalToken + " input: " + inputTokens + " output: " + outputTokens);
            //更新用户token计数和对应
            ...
            CaseSensitiveUtils.appInsights.trackTrace("callWithPicMess 用户 " + userId + " 百炼翻译报错信息 errors ： " + e.getMessage() + " prompt: " + prompt);
            CaseSensitiveUtils.appInsights.trackException(e);
            return null;
        }
    }
        ...
        try {
            GenerationResult call = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                        try {
                            ...
                            CaseSensitiveUtils.appInsights.trackTrace("每日须看 callWithQwenMaxToDes 百炼翻译报错信息 errors ： " + e.getMessage() + " prompt : " + prompt + " 用户：" + userId);
                            CaseSensitiveUtils.appInsights.trackException(e);
                            return null;
                        }
                    },
            ...
            AppInsightsUtils.printTranslateCost(totalToken, inputTokens, outputTokens);
            iapgUserCounterService.updateUserUsedCount(userId, totalToken, userMaxLimit);
            CaseSensitiveUtils.appInsights.trackTrace("用户 token ali-max : " + content + " all: " + totalToken + " input: " + inputTokens + " output: " + outputTokens);
            countUtils.addChars(totalToken);
        } catch (Exception e) {
            CaseSensitiveUtils.appInsights.trackTrace("callWithQwenMaxToDes 百炼翻译报错信息 errors ： " + e.getMessage() + " prompt: " + prompt);
            return null;
        }
        return content;
        ...
        try {
            GenerationResult call = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                        try {
                            ...
                            CaseSensitiveUtils.appInsights.trackTrace("每日须看 textTranslate 百炼翻译报错信息 errors ： " + e.getMessage() + " translateText : " + text + " 用户：" + shopName);
                            CaseSensitiveUtils.appInsights.trackException(e);
                            return null;
                        }
                    },
            ...
            Integer inputTokens = call.getUsage().getInputTokens();
            Integer outputTokens = call.getUsage().getOutputTokens();
            CaseSensitiveUtils.appInsights.trackTrace("textTranslate " + shopName + " 用户 原文本：" + text + " 翻译成： " + content + " token ali: " + content + " all: " + totalToken + " input: " + inputTokens + " output: " + outputTokens);
            AppInsightsUtils.printTranslateCost(totalToken, inputTokens, outputTokens);
//            pcUsersRepo.updateUsedPointsByShopName(shopName, PCUserPicturesService.APP_ALT_FEE);

        } catch (Exception e) {
            CaseSensitiveUtils.appInsights.trackTrace("textTranslate 百炼翻译报错信息 errors ： " + e.getMessage() + " translateText : " + text);
            CaseSensitiveUtils.appInsights.trackException(e);
            return null;
        }
        return content;
