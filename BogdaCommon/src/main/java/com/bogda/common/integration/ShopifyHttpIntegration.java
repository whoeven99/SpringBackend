        ...
        try {
            CloseableHttpResponse response = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                        try {
                            ...
                            CaseSensitiveUtils.appInsights.trackTrace("每日须看 sendShopifyPost api报错信息 errors ： " + e.getMessage() + " url : " + url + " 用户：" + shopName);
            ...
            CaseSensitiveUtils.appInsights.trackTrace("sendShopifyPost api报错信息 errors ： " + e.getMessage() + " url : " + url + " 用户：" + shopName);
            CaseSensitiveUtils.appInsights.trackException(e);
            return null;
        }
        ...
        try {
            ...
            CloseableHttpResponse response = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                        try {
                            ...
                            CaseSensitiveUtils.appInsights.trackTrace("每日须看 sendShopifyPost api报错信息 errors ： " + e.getMessage() + " url : " + url + " 用户：" + request.getShopName());
                            CaseSensitiveUtils.appInsights.trackException(e);
                            return null;
                        }
                    },
            ...
            CaseSensitiveUtils.appInsights.trackTrace("sendShopifyPost api报错信息 errors ： " + e.getMessage() + " url : " + url + " 用户：" + request.getShopName());
            CaseSensitiveUtils.appInsights.trackException(e);
            return null;
        }
        return responseContent;
    ...
    public static JSONObject getInfoByShopify(ShopifyRequest shopifyRequest, String query) {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                CaseSensitiveUtils.appInsights.trackException(e);
                CaseSensitiveUtils.appInsights.trackTrace("getInfoByShopify Shopify request failed on attempt " + attempt + ": " + e.getMessage());
            }
        ...
        if (response == null) {
            CaseSensitiveUtils.appInsights.trackTrace("getInfoByShopify Failed to get response from Shopify after " + maxRetries + " attempts.");
            return null;
        }
    ...
    public static String registerTransaction(ShopifyRequest request, Map<String, Object> variables) {
        ...
        for (int i = 0; i < retryCount; i++) {
            try {
                ...
                CaseSensitiveUtils.appInsights.trackException(e);
                CaseSensitiveUtils.appInsights.trackTrace("每日须看 registerTransaction errors : " + "用户： " + request.getShopName() + " 目标： " + request.getTarget() + "  " + e.getMessage());
            }
            ...
            try {
                ...
                CaseSensitiveUtils.appInsights.trackTrace("每日须看 registerTransaction Thread sleep interrupted: " + e.getMessage());
            }
        }
