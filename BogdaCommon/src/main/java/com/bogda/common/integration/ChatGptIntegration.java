...
//                .buildClient();
    ...
    public Pair<String, Integer> chatWithGpt(String prompt, String sourceText, String shopName, String target) {
        ...
        try {
            ...
            CaseSensitiveUtils.appInsights.trackTrace("ChatGptIntegration 用户： " + shopName + " 翻译语言： " + target + " 翻译的文本： " +
                    sourceText + " token openai : " + content + " all: " + allToken);
            return new Pair<>(content, allToken);
        } catch (Exception e) {
            CaseSensitiveUtils.appInsights.trackTrace("ChatGptIntegration " + shopName + " chatWithGpt error: " + e.getMessage() + " sourceText: " + sourceText + " prompt: " + prompt);
