    ...
    public boolean sendEmailByTencent(Long templateId, String subject, Map<String, String> templateData, String from, String to) {
        ...
        try {
            ...
            if (resp == null || resp.getRequestId() == null) {
                CaseSensitiveUtils.appInsights.trackTrace("FatalException 腾讯邮件报错信息 errors ： response is null or RequestId is null, res: " + JsonUtils.objectToJson(resp)
                        + " templateId : " + templateId + " from: " + from + " to: " + to);
                return false;
            }

            CaseSensitiveUtils.appInsights.trackTrace("腾讯邮件信息 ： response is : " + JsonUtils.objectToJson(resp) + " templateId : " + templateId + " from: " + from + " to: " + to + " templateData : " + JsonUtils.objectToJson(templateData));
            return true;
        } catch (Exception e) {
            CaseSensitiveUtils.appInsights.trackException(e);
            CaseSensitiveUtils.appInsights.trackTrace("FatalException 腾讯邮件报错信息 errors ： " + e.getMessage() + " templateId : " + template + " from: " + from + " to: " + to);
            return false;
        }
    }
...
//            httpProfile.setEndpoint("ses.ap-guangzhou-open.tencentcloudapi.com");
            ...
            SendEmailResponse resp = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                        try {
                            return client.SendEmail(req);
                        } catch (Exception e) {
                            CaseSensitiveUtils.appInsights.trackTrace("FatalException sendEmailByTencent 腾讯邮件报错信息 errors ： " + e.getMessage() + " templateId : " + tencentSendEmailRequest.getTemplateId() + " data" + templateData.toString());
                            CaseSensitiveUtils.appInsights.trackException(e);
                            return null;
                        }
                    },
            ...
            if (resp == null) {
                CaseSensitiveUtils.appInsights.trackTrace("FatalException sendEmailByTencent 腾讯邮件报错信息 errors ： templateId : " + tencentSendEmailRequest.getTemplateId() + " data" + templateData.toString());
                return false;
            }

            // 输出json格式的字符串回包
            jsonString = AbstractModel.toJsonString(resp);
            CaseSensitiveUtils.appInsights.trackTrace("sendEmailByTencent 腾讯邮件信息 jsonString : " + jsonString + " data: " + tencentSendEmailRequest.getTemplateData());
        } catch (Exception e1){
            CaseSensitiveUtils.appInsights.trackException(e1);
            CaseSensitiveUtils.appInsights.trackTrace("FatalException sendEmailByTencent 腾讯邮件报错信息 errors ： " + e1.getMessage() + " templateId : " + tencentSendEmailRequest.getTemplateId() + " data" + templateData.toString());
        }
