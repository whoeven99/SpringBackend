...
public class HunYuanBucketIntegration {
    ...
    private static void showTransferProgress(Transfer transfer) {
        CaseSensitiveUtils.appInsights.trackTrace(transfer.getDescription());
        do {
            try {
                Thread.sleep(2000);
            ...
            CaseSensitiveUtils.appInsights.trackTrace("[" + soFar + " / " + total + "] = " + pct);
        } while (transfer.isDone() == false);
        CaseSensitiveUtils.appInsights.trackTrace(String.valueOf(transfer.getState()));
    }
    ...
    public static String uploadFile(MultipartFile file, String shopName, String imageId) {
            ...
            try {
                PutObjectRequest putObjectRequest = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                            try {
                                return new PutObjectRequest(BUCKET_NAME, key, file.getInputStream(), metadata);
                            } catch (Exception e) {
                                CaseSensitiveUtils.appInsights.trackTrace("每日须看 uploadFile 腾讯上传图片报错信息 errors ： " + e.getMessage() + " imageId : " + imageId + " 用户：" + shopName);
...
//                appInsights.trackTrace("used time: " + (endTime - startTime) / 1000);
                ...
                CaseSensitiveUtils.appInsights.trackTrace("每日须看 uploadFile 腾讯上传图片报错信息 errors : " + e.getMessage() + ", shopName: " + shopName + " imageId: " + imageId);
            } finally {
                transferManager.shutdownNow();
            }
        ...
        try {
            PutObjectRequest putObjectRequest = TimeOutUtils.callWithTimeoutAndRetry(() -> {
                        try {
                            ...
                            CaseSensitiveUtils.appInsights.trackTrace("每日须看 uploadFile 腾讯上传图片报错信息 errors ： " + e.getMessage() + " key : " + key );
                            CaseSensitiveUtils.appInsights.trackException(e);
                            return null;
                        }
                    },
