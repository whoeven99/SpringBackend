...
import com.bogda.common.mapper.APGUserGeneratedSubtaskMapper;
...
public class APGUserGeneratedSubtaskServiceImpl extends ServiceImpl<APGUserGeneratedSubtaskMapper, APGUserGeneratedSubtaskDO> implements IAPGUserGeneratedSubtaskService {
    ...
    public Boolean updateStatusById(String subtaskId, int i) {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if (Boolean.TRUE.equals(b)) {
                    ...
                    retryCount++;
                    CaseSensitiveUtils.appInsights.trackTrace("updateStatusById 更新失败（返回false） errors ，准备第" + retryCount + "次重试，shopName=" + subtaskId);
                }
            } catch (Exception e) {
                retryCount++;
                CaseSensitiveUtils.appInsights.trackTrace("updateStatusById 更新失败（抛异常） errors ，准备第" + retryCount + "次重试，shopName=" + subtaskId + ", 错误=" + e);
            }
        ...
        CaseSensitiveUtils.appInsights.trackTrace("updateStatusById 更新失败 errors ，重试" + maxRetries + "次后仍未成功，shopName=" + subtaskId);
        return false;
    }
