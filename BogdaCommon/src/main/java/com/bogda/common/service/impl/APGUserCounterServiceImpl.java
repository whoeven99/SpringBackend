...
import com.bogda.common.mapper.APGUserCounterMapper;
...
//        return baseMapper.selectOne(new QueryWrapper<APGUserCounterDO>().eq("shop_name", shopName));
    ...
    public Boolean updateUserUsedCount(Long userId, Integer counter, Integer maxLimit) {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if (Boolean.TRUE.equals(b)) {
                    ...
                    retryCount++;
                    CaseSensitiveUtils.appInsights.trackTrace("updateUserUsedCount 更新失败（返回false） errors ，准备第" + retryCount + "次重试，shopName=" + userId);
                }
            } catch (Exception e) {
                retryCount++;
                CaseSensitiveUtils.appInsights.trackTrace("updateUserUsedCount 更新失败（抛异常） errors ，准备第" + retryCount + "次重试，shopName=" + userId + ", 错误=" + e);
            }
        ...
        CaseSensitiveUtils.appInsights.trackTrace("updateUserUsedCount 更新失败 errors ，重试" + maxRetries + "次后仍未成功，shopName=" + userId);
        return false;
