...
import com.bogda.common.mapper.TranslationCounterMapper;
...
public class TranslationCounterServiceImpl extends ServiceImpl<TranslationCounterMapper, TranslationCounterDO> implements ITranslationCounterService {
    ...
    public Boolean updateCharsByShopName(String shopName, String accessToken, String gid, Integer chars) {
        ...
        if (gid.contains("AppPurchaseOneTime")){
            CaseSensitiveUtils.appInsights.trackTrace("一次性购买 用户： " + shopName);
            query = ShopifyRequestBody.getSingleQuery(gid);
        }else {
            CaseSensitiveUtils.appInsights.trackTrace("计划购买 用户： " + shopName);
            query = ShopifyRequestBody.getSubscriptionQuery(gid);
        }
        CaseSensitiveUtils.appInsights.trackTrace("updateCharsByShopName 用户： " + shopName + " query: " + query);
        ...
        if (queryValid == null) {
            CaseSensitiveUtils.appInsights.trackTrace("updateCharsByShopName " + shopName + " 用户  errors queryValid : " + queryValid);
            return false;
        }
    ...
    public Boolean deleteTrialCounter(String shopName) {
        ...
        if (translationCounterDO != null && translationCounterDO.getOpenAiChars() == 1){
            CaseSensitiveUtils.appInsights.trackTrace("deleteTrialCounter " + shopName + " 用户 删除试用额度 " + translationCounterDO.getGoogleChars());
