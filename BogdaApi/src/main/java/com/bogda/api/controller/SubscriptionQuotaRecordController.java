...
public class SubscriptionQuotaRecordController {
    ...
    public void addSubscriptionQuotaRecord(@RequestBody SubscriptionQuotaRecordDO subscriptionQuotaRecordDO) {
        ...
        CaseSensitiveUtils.appInsights.trackTrace("addSubscriptionQuotaRecord " + subscriptionQuotaRecordDO.getSubscriptionId() + " 这条数据只是判断是否要往数据库存值 byId: " + byId);
        if (byId != null){
            CaseSensitiveUtils.appInsights.trackTrace("addSubscriptionQuotaRecord " + subscriptionQuotaRecordDO.getSubscriptionId() + " 数据库中含有这条数据，就不插入了");
            return;
        }
        // 重试逻辑
        while (attempt < maxRetries && !success) {
            try {
                ...
                if (i != null && i == 1) {
                    ...
                    attempt++;
                    CaseSensitiveUtils.appInsights.trackTrace("addSubscriptionQuotaRecord " + subscriptionQuotaRecordDO.getSubscriptionId() + " 插入结果不符合预期 (i=" + i + ")，正在重试第 " + (attempt + 1) + " 次");
                }
            } catch (Exception e) {
                attempt++;
                CaseSensitiveUtils.appInsights.trackTrace("addSubscriptionQuotaRecord " + subscriptionQuotaRecordDO.getSubscriptionId() + " 插入异常，正在重试第 " + (attempt + 1) + " 次: " + e.getMessage());
            }
        }
